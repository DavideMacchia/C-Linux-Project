...
int main(){
	...
	if(pid == -1){
		...
		/*Contatore numero pazienti presenti nel file pazienti.txt*/
		int counter_paz = 0;
		if((file_paz = fopen("pazienti.txt", "r")) == NULL){
			perror("Errore all'apertura del file \"pazienti.txt\"\n");
			exit(EXIT_FAILURE);
		}
		...
		if(fclose(file_paz) == EOF){
			perror("Errore durante la chiusura del file \"pazienti.txt\"\n");
			exit(EXIT_FAILURE);
		}
		
		
		/*Contatore numero sintomi presenti nel file sintomi.txt*/
		int counter_sint = 0;
		if((file_sint = fopen("sintomi.txt", "r")) == NULL){
			perror("Errore durante l'apertura del file \"sintomi.txt\"\n");
			exit(EXIT_FAILURE);
		}
		...
		if(fclose(file_sint) == EOF){
			perror("Errore durante la chiusura del file \"sintomi.txt\"\n");
			exit(EXIT_FAILURE);
		}
		...
		int paz_inseriti = 1;
		while(1){
			/*Si inizializzano paziente.nome e paziente.cognome con un paziente selezionato casualmente dal file pazienti.txt*/
			srand(time(NULL));
			...
			if(fscanf(file_paz, "%s %s", paziente.nome, paziente.cognome) != 2){
				perror("Errore durante la lettura del file \"pazienti.txt\"\n");
				exit(EXIT_FAILURE);
			}
			if(fclose(file_paz) == EOF){
				perror("Errore durante la chiusura del file \"pazienti.txt\"\n");
			...
			/*Si inizializza paziente.sintomo con un sintomo selezionato casualmente dal file sintomi.txt*/
			srand(time(NULL));
			rand_sint = (rand() % counter_sint) + 1;
			
			if((file_sint = fopen("sintomi.txt", "r")) == NULL){
				perror("Errore all'apertura del file \"sintomi.txt\"\n");
				exit(EXIT_FAILURE);
			}
			j = 0;
			while(j != rand_sint - 1){
			...
			if(fscanf(file_sint, "%s %*d", paziente.sintomo) != 1){
				perror("Errore durante la lettura del file \"sintomi.txt\"\n");
				exit(EXIT_FAILURE);
			}
			if(fclose(file_sint) == EOF){
				perror("Errore durante la chiusura del file \"sintomi.txt\"\n");
			...
			/*STAMPA A VIDEO PAZIENTE e CREAZIONE FILE DI LOG(ospedale.txt)*/
...
void termina(){
	printf("\nTEMPO SCADUTO, attesa che tutti i pazienti escano da prestazione\n\n");
	uscita = 0;
...
void pulisci(){
		...
		if(msgctl(coda_id, IPC_RMID, 0) == -1){
			perror("Errore nel msgctl durante la rimozione coda ospedale-triage\n");
			exit(EXIT_FAILURE);
		}
...
void clean_file_log(){
		//Eliminazione file di log ospedale
